...
| `KINDLE_EMAIL` | none | Listed as an environment-backed downloads setting in generated project docs, but the current Send-to-Kindle route does **not** read it. The route instead reads the per-user database field `kindle_address`; if absent, it returns HTTP 400 `No email recipient configured`. Treat `KINDLE_EMAIL` as currently ineffective for this endpoint. [Source: `docs/environment-variables.md:396-405,465-472`; `shelfmark/config/settings.py:763-774`; `shelfmark/core/library_routes.py:523-532`; `shelfmark/core/user_db.py:611-668`] |
...
Do not put a user's Kindle address in the example as `KINDLE_EMAIL`: current source does not consume it for Send-to-Kindle. Each user must instead save `kindle_address` through the self-settings UI or `PUT /api/users/me`; Shelfmark persists it in `user_preferences`. Therefore environment variables alone do **not** complete Send-to-Kindle setup. [Source: `src/frontend/src/components/settings/SelfSettingsModal.tsx:242-251`; `shelfmark/core/self_user_routes.py:24-29,118-191`; `shelfmark/core/user_db.py:611-668`]
