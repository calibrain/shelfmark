...
// Legacy result cards can still be rendered while an already-open browser tab
// transitions to the library shell. They do not expose a standalone route.
export const getBrowseSource = (book: Book): string => {
  const source = book.source || book.provider;
  if (!source) throw new Error(`Book ${book.id} is missing source context`);
  return source;
};
...
export const buildDirectRequestPayload = (book: Book): CreateRequestPayload => {
  ...
  return {
    ...
    context: { source, content_type: 'ebook', request_level: 'release' },
  };
};
